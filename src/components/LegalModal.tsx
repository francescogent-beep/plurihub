...
function PrivacyContent() {
  return (
    <>
      ...
      <P>PluriHub is a Chrome extension developed and operated by an individual developer based in Spain. For any privacy-related questions, contact us at: <a href="mailto:info@plurihub.com" className="text-[#0EA5E9] hover:underline">info@plurihub.com</a></P>
      ...
      <SectionTitle>7. Header Modification</SectionTitle>
      <P>To display AI provider interfaces (Claude, ChatGPT, Gemini, etc.) inside the Chrome side panel, PluriHub removes certain HTTP response headers (X-Frame-Options, Content-Security-Policy) from the listed AI provider domains. This is required purely for technical embedding purposes. PluriHub does not read, intercept, or modify the actual content of any request or response — only the framing-related security headers are affected.</P>
...
function TermsContent() {
  return (
    <>
      ...
      <P>PluriHub offers a free trial period followed by a paid Pro plan. Subscription fees are charged in advance on a monthly or annual basis via Stripe. Fees are non-refundable except where required by applicable law. We reserve the right to modify pricing with 30 days notice to active subscribers.</P>
      ...
      {doc && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative z-10 bg-[#0f0f0f] border border-[#1e1e1e] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e1e1e] flex-shrink-0">
              <div>
                <p className="text-[#0EA5E9] text-[10px] font-mono uppercase tracking-widest mb-0.5">PluriHub</p>
                <h2 className="text-white font-semibold text-lg">{TITLES[doc]}</h2>
              </div>
              <button
                onClick={onClose}
                className="w-8 h-8 rounded-lg border border-[#2a2a2a] flex items-center justify-center text-[#52525b] hover:text-white hover:border-[#3a3a3a] transition-colors"
                aria-label="Close"
              >
                <X size={14} />
              </button>
            ...
            {/* Scrollable body */}
            <div className="overflow-y-auto px-6 py-6 flex-1 scroll-smooth">
              {doc === 'privacy' ? <PrivacyContent /> : <TermsContent />}

              <p className="text-[#2a2a2a] text-xs text-center mt-10 pt-6 border-t border-[#1a1a1a]">
